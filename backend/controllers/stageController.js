...
const searchStages = asyncHandler(async (req, res) => {
  ...
  const getStagesByEntrepriseId = asyncHandler(async (req, res) => {
    try {
      ...
      const stages = await Stage.find({ entreprise: entrepriseId });
      res.json(stages);
    } catch (error) {
      res.status(500).json({ message: error.message });
    }
  });
  const getStageByEntrepriseId = asyncHandler(async (req, res) => {
    try {
      const { entrepriseId } = req.params;
      const stages = await Stage.findOne({ entreprise: entrepriseId });
      res.json(stages);
    } catch (error) {
      res.status(500).json({ message: error.message });
    }
