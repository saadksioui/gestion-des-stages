...
const ChatR = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `http://127.0.0.1:8000/api/responsable/getResponsableById/${storedId[1]}`
        );
      ...
      try {
        const usersData = await Promise.all(
          userIds.map(async (userId) => {
            const response = await axios.get(
              `http://127.0.0.1:8000/api/auth/findById/${userId}`
            );
  ...
  const showChat = useCallback(
    async (id) => {
      try {
        const response = await axios.get(
          "http://127.0.0.1:8000/api/suivi/get_etud",
          {
            params: {
              id_étudiant: id,
            },
        ...
        const userResponse = await axios.get(
          `http://127.0.0.1:8000/api/auth/findById/${id}`
        );
  ...
  const handleMsg = async (e) => {
    ...
    try {
      const response = await axios.post(
        `http://127.0.0.1:8000/api/suivi/send/${chatId}`,
        {
  ...
  const handleMsgDelete = async (e, id) => {
    ...
    try {
      await axios.put(
        `http://127.0.0.1:8000/api/suivi/deleteChatMessage/${id}`,
        { id: chatId }
      );
    ...
    <UserLayout>
      <div className="p-10 w-full h-full flex items-center gap-10">
        ...
        <div className="chatbox w-2/3 h-full flex flex-col gap-5">
          <div className="chat h-[90%] border border-[#999999] rounded-t-xl relative">
            <div className="user h-[25%] p-3 border-b-[0.5px] flex items-center gap-5">
              {selectedUser && (
                <>
                  <img
                    src={`images_cv/${selectedUser.img_url}`}
                    className="size-20 rounded-full object-cover"
            ...
            <button
              ...
              className="text-xl text-gray-700 mt-2 hover:text-gray-900"
          ...
          <div className="input w-full h-[10%]">
            <form
              onSubmit={handleMsg}
              className="w-full h-full flex justify-between gap-5 items-center px-3 border border-[#D6D6D6] rounded-xl bg-[#F6F6F6]"
            >
              <input
                type="text"
                value={newMsg}
                onChange={(e) => setNewMsg(e.target.value)}
                className="w-[90%] outline-none h-full rounded-xl bg-[#F6F6F6] placeholder:text-[#999999]"
