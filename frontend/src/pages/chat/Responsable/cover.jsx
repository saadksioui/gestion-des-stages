...
const ChatR = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response1 = await axios.get(`http://127.0.0.1:8000/api/auth/findById/${Profile}`);
        setUser(response1.data);
        setUserid(response.data.ids_stg);
      } catch (error) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const usersData = await Promise.all(Userid.map(async (userId) => {
  ...
  const handleMsg = async (e) => {
    ...
    try {
      ...
      const updatedMessages = [...messages, response.data]; // Assuming the API returns the newly added message
        ...
        <div className="users rounded-xl border h-full border-[#999999] w-1/3 flex flex-col overflow-y-scroll">
          {Users.map((user, i) => (
            <div className="user" key={i} >
              <a onClick={() => ShowChat(user._id)} className='flex items-center gap-5 border-b-[0.5px] border-[#999999] p-3'>
