  ...
  const handleSearch = (e) => {
    ...
    if (!searchTitle) {
      ...
      const filtered = users.filter(
        (user) =>
          user.type_utilisateur === "étudiant" && user.nom.toLowerCase().includes(searchTitle.toLowerCase())
      );
      ...
      <section className="px-10 mt-10">
        ...
        <div className="my-6 flex items-center justify-center">
          <form
            ...
            className="w-[425px] h-[72px] flex justify-between items-center px-3 border border-[#D6D6D6] rounded-xl bg-[#F6F6F6]"
          >
            <input
              type="text"
            ...
            />
            <button type="submit" className="p-3 text-white bg-black rounded-xl">
              Rechercher
            </button>
          </form>
        </div>
        ...
        <div className="lg:hidden grid grid-cols-1 gap-4 mb-10 p-4 bg-white rounded-lg border shadow-md">
          <div className="mb-4">
            <h3 className="text-xl font-bold leading-none text-gray-900">Vos stagiaires</h3>
          </div>
          <ul role="list" className="divide-y divide-gray-200">
            {filteredStages.map((stagiaire) => (
              <li key={stagiaire._id} className="py-3 sm:py-4">
                <div className="flex items-center space-x-4">
                  <div className="flex-shrink-0">
                    <img
                      className="w-8 h-8 rounded-full object-cover"
                      src={`/images_cv/${stagiaire.img_url}`}
                      alt={`${stagiaire.nom} image`}
                    />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-gray-900">{stagiaire.nom}</p>
                    <p className="text-sm text-gray-500 truncate">{stagiaire.email}</p>
                  </div>
                </div>
              </li>
            ))}
